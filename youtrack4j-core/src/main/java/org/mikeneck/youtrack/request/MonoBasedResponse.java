...
public class MonoBasedResponse<R> implements ApiResponse<R> {
    ...
    public MonoBasedResponse(final Mono<R> mono) {
        this.mono = mono;
    }
    ...
    public <S> ApiResponse<S> next(final Handler.NextRequest<? super R, ? extends S> nextRequest) {
        final Mono<S> next = Mono.create(sink -> mono.map(nextRequest::handle)
                .subscribe(response -> response.onSuccess(sink::success)
                        .onFailure(res -> {
                            if (res instanceof ApiException) {
                                sink.error((ApiException)res);
                            } else {
                                sink.error(new ApiException(res.getStatusCode(), res.getBody()));
                            }
                        })));
    ...
    public ApiResponse<R> onFailure(final Handler.FailureHandler failureHandler) {
        final Mono<R> next = mono.doOnError(e -> {
